...
/*   Updated: 2024/09/28 03:23:35 by msitni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3	shift_by_normal_map(t_object *obj, t_hit hit, t_ray ray)
{
	...
	if (vec3_magnitude(u_vec) > ZERO)
	{
		...
		pixel = vec3_scale(get_vec3_color(get_img_pixel_at(&obj->normal_map,
						pixel.x, pixel.y)), 1 / 255.0f);
		normal = vec3_add_vec3(vec3_scale(u_vec, (pixel.x * 2 - 1)),
				vec3_scale(v_vec, (pixel.y * 2 - 1)));
...
int	is_not_in_shadow(t_scene *scene, t_light *light, t_hit hit)
{
	...
	light_ray.dir = vec3_normalize(vec3_sub_vec3(light_ray.target,
				light_ray.origin));
...
t_vec3	shade_color(t_scene *scene, t_hit hit, t_ray ray)
{
	...
	while (i < scene->lights_count)
	{
		...
		if (is_not_in_shadow(scene, light, hit))
		{
			...
			shaded_color = vec3_add_vec3(shaded_color, vec3_scale(light->color,
						intensity));
		}
		i++;
	}
