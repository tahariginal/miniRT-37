...
/*   Updated: 2024/09/28 03:26:06 by msitni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3	calculate_reflection_color(t_scene *scene, t_hit *hit,
		t_vec3 *hit_color, t_vec3 *ray_dir)
{
	t_vec3	result;
	t_ray	r_ray;
	t_hit	r_hit;
...
void	render_pass(t_renderer *r)
{
	...
	while (y > 0)
	{
		...
		while (x < r->mlx_texture.width)
		{
			...
			c = calculate_intersections(&r->scene, ray);
			set_img_pixel_at(&r->mlx_texture, x, r->mlx_texture.height - y, c);
			x++;
		}
		y--;
	}
	mlx_put_image_to_window(r->mlx_context, r->window, r->mlx_texture.handle, 0,
		0);
}
